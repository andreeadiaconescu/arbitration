...
for iSubj = iSubjectArray
    ...
    switch typeDesign
        case {'ModelBased','ModelBasedwithDifference'}
            ...
            Social_weighting     = zscore([0.5; wx.*mu1hat_a]);
            Card_weighting       = zscore([0.5; wc.*mu1hat_r]);
            Arbitration          = zscore([0.5; wc]);
            
            pmod(1,1).param      = {[Arbitration],[Social_weighting],[Card_weighting],zscore([4;px]),zscore([4;pc])}; % Precision (Model-based wager)
            pmod(1,1).poly       = {[1],[1],[1],[1],[1]};
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % Second: Wager (Belief Precision), Belief
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            pmod(1,2).name  = {'BeliefUncertainty','Belief','Trial Number','Wager_Amount'}; % Belief Precision
            pib             = (b.*(1-b));
            pmod(1,2).param = {zscore([4; pib]),zscore([0.5; b]),zscore([1:size(b,1)+1]),zscore([actual_responses(:,2)])}; % Precision (Model-based wager)
            ...
            pmod(1,3).param       = {zscore([0; Epsilon2.Advice]),zscore([0; Epsilon2.Reward]),zscore([0; Epsilon3.Advice]),zscore([0; Epsilon3.Reward])};
