...
%initialize values
nTrials = 39;
th_t=0.2; %1.6094; %log(0.2); %5 %% Change volatility (high to low)
ka_t=1;
om_t=-2;
...
m_t = 1.2; % 1.5 is 81.8%
phi = 0.8; % set to 0 for no drift; set to negative values to drift away from the mean

for k=1:nTrials
    x2_t(k+1)=x2_t(k)+phi.*(m_t-x2_t(k))+randn().*sigma2_t;
    theta_t(k+1) = sgm(x2_t(k+1), 1);      
       t(k+1)=(rand()<=theta_t(k+1));      
end
figure;
hs(1) = subplot(2,1,1);
stem(t, 'm--');
...
linkaxes(hs, 'x');
t=t';
save(fullfile(pathname,'seq_stable_reward.txt'),'t', '-ascii', '-tabs');
save(fullfile(pathname,'x1_t.mat'),'theta_t', '-mat', '-tabs');
save(fullfile(pathname,'x2_t.mat'),'x2_t', '-mat', '-tabs');
end