...
pvec(2)         = exp(ptrans(2));              % sa2r_0 (transform)
pstruct.sa2r_0  = pvec(2);
pvec(3)         = ptrans(3);                   % mu3r_0
pstruct.mu3r_0  = pvec(3);
pvec(4)         = exp(ptrans(4));                % sa3r_0 (transform)
pstruct.sa3r_0  = pvec(4);
pvec(5)         = sgm(ptrans(5),r.c_prc.kaub_r); % ka_r (transform)
pstruct.ka_r    = pvec(5);
pvec(6)         = ptrans(6);                   % om_r
pstruct.om_r    = pvec(6);
pvec(7)         = sgm(ptrans(7),r.c_prc.thub_r); % th_r (transform)
pstruct.th_r    = pvec(7);

pvec(8)         = ptrans(8);                   % mu2a_0
pstruct.mu2a_0  = pvec(8);
pvec(9)         = exp(ptrans(9));              % sa2a_0 (transform)
pstruct.sa2a_0  = pvec(9);
pvec(10)        = ptrans(10);                   % mu3a_0
pstruct.mu3a_0  = pvec(10);
pvec(11)        = exp(ptrans(11));                % sa3a_0 (transform)
pstruct.sa3a_0  = pvec(11);
pvec(12)        = sgm(ptrans(12),r.c_prc.kaub_a); % ka_a (transform)
pstruct.ka_a    = pvec(12);
pvec(13)        = ptrans(13);                   % om_a
pstruct.om_a    = pvec(13);
pvec(14)        = sgm(ptrans(14),r.c_prc.thub_a); % th_a (transform)
pstruct.th_a    = pvec(14);

pvec(15)       = sgm(ptrans(15),1); % phi_r (transform)
pstruct.phi_r  = pvec(15);
pvec(16)       = sgm(ptrans(16),1); % phi_a (transform)
pstruct.phi_a  = pvec(16);
pvec(17)       = ptrans(17); % m_r
pstruct.m_r    = pvec(17);
pvec(18)       = ptrans(18); % m_a
pstruct.m_a    = pvec(18);
