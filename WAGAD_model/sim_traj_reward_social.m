...
% Modify the zeta (amount of social bias)
% Modify the omega

omega_r_parArray= [-3.5:-0.5:-6.5];
omega_a_parArray= [-3.5:-0.5:-6.5];
zeta_parArray=exp([0:0.1429:1])
kappa_r_parArray= 1;% 
kappa_a_parArray= 1; % 

P=numel(omega_r_parArray);
...
for m=1:numel(rp_model)
    for i=1:numel(prc_model)
        ...
        for par=1:O
                ...
                om_r=omega_r_parArray(par);
                om_a=omega_a_parArray(par);
                ka_r=kappa_r_parArray(1);
                ka_a=kappa_a_parArray(1);
                p_prc=[0 1 1 1 ka_r om_r 0.5 0 1 1 1 ka_a om_a 0.5];
                ze=zeta_parArray(par);
                p_obs=[exp(ze) log(48) tapas_sgm((1./(0.99.*.01)-4),1)];
                lgstr_r{par} = sprintf('\\zeta = %3.1f', log(ze));
...
%         % hold off;
        ...
        if ~isempty(find(strcmp(fieldnames(sim),'y'))) && ~isempty(sim.y)
            ...
            title(['Input u (green), responses (rainbow) and cue-related (rainbow) and advice-related (bone) posterior expectation of input s(\mu_2) for \omega cue=', num2str(sim.p_prc.om_r), ...
                ...
                ', \zeta=', num2str(log(sim.p_obs.ze1))],'FontWeight', 'bold');
            
            ylabel('y, u, s(\mu_2)');
            axis([0 t -0.35 1.35]);
        else
            title(['Input u (green) and posterior expectation of input s(\mu_2) (red) for \omega cue=', ...
                num2str(sim.p_prc.om_r), ', \omega advice=', num2str(sim.p_prc.om_a), ', \zeta=', num2str(log(sim.p_obs.ze1))], ...
                'FontWeight', 'bold');
            ylabel('u, s(\mu_2)');
            axis([0 t -0.1 1.1]);
        end
