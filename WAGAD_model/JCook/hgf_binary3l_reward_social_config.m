...
% Upper bound for kappa and theta (lower bound is always zero)     JC - theta should be <1, kappa should be <2
c.kaub_r = 0.5;%2;
c.thub_r = 0.5;%0.5;
c.kaub_a = 0.5;%2;
c.thub_a = 0.5;%0.5;
...
% ++++++++ Priors for full HGF ++++++++++ Jen Cook 28 june
% Initial mu2
...
% Initial sigma3
c.logsa3r_0mu = log(0.8);
c.logsa3r_0sa = 1;
c.logsa3a_0mu = log(0.8);
c.logsa3a_0sa = 1;

% Kappa
% This should be fixed (preferably to 1) if the observation model
% does not use mu3 (kappa then determines the scaling of x3).
% JC - try the following: 0.0200    0.1200    0.2200    0.3200    0.4200    0.5200    0.6200    0.7200
c.logitkamu_r = 0.25;
c.logitkasa_r = 0;
c.logitkamu_a = 0.25;
c.logitkasa_a = 0;

% Omega    JC - try the following: -3.5000   -4.0000   -4.5000   -5.0000   -5.5000   -6.0000   -6.5000
c.ommu_r = -4;
c.omsa_r = 5^2;
c.ommu_a = -4;
c.omsa_a = 5^2;

% Theta, this is also fixed to 0.5     JC - should be half of upper bound
% try: 0    0.0500    0.1000    0.1500    0.2000    0.2500    0.3000    0.3500    0.4000    0.4500    0.5000
c.logitthmu_r = 0.25;%0;
c.logitthsa_r = 0;%0;
c.logitthmu_a = 0.25;%0;
c.logitthsa_a = 0;%0;
% 
