...
for k = 2:1:n
    if not(ismember(k-1, r.ign))
        ...
        % ~~~~~~~~~
        % Precision of prediction
        pi2hat_r(k) = 1/(1/pi2_r(k-1) +exp(ka_r *mu3_r(k-1) +om_r));
        pi2hat_a(k) = 1/(1/pi2_a(k-1) +exp(ka_a *mu3_a(k-1) +om_a));
        ...
        % ~~~~~~~~~
        % Precision of prediction
        pi3hat_r(k) = 1/(1/pi3_r(k-1) +th_r);
        pi3hat_a(k) = 1/(1/pi3_a(k-1) +th_a);

        % Weighting factor
        w2_r(k) = exp(ka_r *mu3_r(k-1) +om_r) *pi2hat_r(k);
        w2_a(k) = exp(ka_a *mu3_a(k-1) +om_a) *pi2hat_a(k);
...
infStates = NaN(n-1,l,4); % trials, levels, trajectories
