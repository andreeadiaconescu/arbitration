function tapas_rw_social_reward_plotTraj(r)
...
% Plot
plot(0:t, [r.p_prc.vr_0; r.traj.v_r], 'r', 'LineWidth', 2);
hold all;
plot(0:t, [r.p_prc.va_0; r.traj.v_a], 'm', 'LineWidth', 2);

plot(0, r.p_prc.vr_0, 'or', 'LineWidth', 2); % prior
...
if ~isempty(find(strcmp(fieldnames(r),'y'))) && ~isempty(r.y)
    ...
    title(['Response y (orange), input u (green), and value v card (red), advice (magenta) for alpha_card=', ...
           num2str(r.p_prc.al_r), ', alpha_advice=', num2str(r.p_prc.al_a)], 'FontWeight', 'bold');
    ylabel('y, u, v');
    axis([0 t -0.15 1.15]);
else
    title(['Input u (green) and value v card (red), advice (magneta) for alpha_card=', ...
           num2str(r.p_prc.al_r), ', alpha_advice=', num2str(r.p_prc.al_a)], 'FontWeight', 'bold');
    ylabel('u, v');
    axis([0 t -0.1 1.1]);
end
