function SOC = MS_fMRI_Wager_Sess2a_final(subject,run,scanner_mode,exp_task,mode_key)
...
switch language_inst
    case 1
        ...
        %         preparestring('Antworten Sie so schnell und so fehlerfrei wie möglich.',4,0,-80);
        ...
        preparestring('Mit der linken Taste verschieben Sie den Balken rechts,',4,0,80);
        preparestring('Mit der rechten Taste verschieben Sie ihn links.',4,0,40);
        ...
        cgfont('Arial', 24);
        preparestring('Bitte sagen Sie vorher, welche Karte (Grün oder Blau)',8,0,180);
        preparestring('gewählt wird.',8,0,140);
        preparestring('Drücken Sie die rechte mit',8,-250,60);
        ...
        preparestring('Bitte warten, jetzt machen wir eine Pause',15); %end of the experiment
    case 2
        cgfont('Arial', 40);
        preparestring('+',1,0,0);
        cgfont('Arial', 40);
...
cgloadbmp(17,'Reverse_Wager_Bar.bmp');
    ...
    case 2 % main
        switch run
            case {1,3,5,7}
                ...
                % Inputlist fixed cues, targets, ITIs, and blocks
                SOC.Inputlist_S = 'Task_volatility_fMRIa.txt';
                [SOC.Block_codes,SOC.ITI_codes,SOC.Video_codes,SOC.displaystim,SOC.Outcome_codes,SOC.displaytarget] = textread(SOC.Inputlist_S, '%d%d%d%s%d%s');
                SOC.param(exp_task).block_length = [25 15 30]; %specify the total number of trials in the 3 blocks
        end
            ...
            for trial = startTrialBlock + (0:nBlockArray(block)-1) %number of trials provided in script
                ...
                if SOC.mode_key == 1
                    ...
                    while done1 == 0
                        ...
                        cgtext(num2str(11-(current / 40 + 1)),current-200,-200);
                        cgflip(0,0,0);
